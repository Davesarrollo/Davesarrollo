### Hola, soy David Monroy (Davesarrollo) 👋

Soy un desarrollador enfocado en comprender cómo funcionan los programas internamente y en base a ello construir escribir código limpio, eficiente y bien organizado, donde la lógica y la arquitectura realmente importan.

- Actualmente estoy trabajando en **MHUR Damage Calculator**, una aplicación de consola en C.
- Estoy profundizando en **manejo de memoria, modelado de datos complejos y el funcionamiento interno de los programas**.
- En estos momentos trabajo principalmente con **C en Linux**.

---

## Proyecto Destacado

### [MHUR Damage Calculator](https://github.com/Davesarrollo/mhur-damage-calculator)
Aplicación de consola desarrollada en C para visualizar estadísticas e información de personajes del videojuego *My Hero Ultra Rumble*.

Lo que comenzó como una herramienta simple evolucionó hacia una aplicación modular con enfoque en escalabilidad y arquitectura limpia:
- **Diseño Modular:** Separación clara de responsabilidades para facilitar el mantenimiento.
- **Modelado de Datos:** Uso de `structs` para representar de forma clara la estructura de personajes y habilidades..
- **Compilación y Build:** Gestionado con un **Makefile** personalizado para compilación eficiente en entornos Linux y Windows.

---

## Intereses Actuales

- **Desarrollo de Bajo Nivel:** Manejo de memoria, punteros y estructuras de datos en C.
- **Arquitectura de Software:** Organización del código, modularidad y diseño de sistemas.
- **Optimización:** Escribir código eficiente y con buen rendimiento.
- **Entornos Linux:** Desarrollo y herramientas del ecosistema.
